...
class Solver(object):
    ...
    def test_ops(self):
        for batch_idx, batch in enumerate(self.test_dataset):
            with torch.no_grad():
                ...
                for idx in range(self.opt.interpolate_len):# idx from 0 to 4
    ...
    def test_save_imgs(self, faces_list, paths_list):
        print("paths list ==========  ",len(paths_list[0]))#batch size
        for idx in range(len(paths_list[0])):
            ...
            if self.opt.save_test_gif:
                ...
                """
                modification: 
                    original face is concate_img = np.array(self.visual.numpy2im(faces_list[0][idx]))
                    (if there is no interpolation) generated face is concate_img = np.array(self.visual.numpy2im(faces_list[1][idx]))
                    target face is concate_img = np.array(self.visual.numpy2im(faces_list[ last index ][idx]))
                """
#                concate_img = np.array(self.visual.numpy2im(faces_list[0][idx]))
                concate_img = np.array(self.visual.numpy2im(faces_list[len(faces_list)-2][idx]))
                """
                modification:
                    if you want to show all the current generated face, please de-comment the following 3 lines
                """
                print("faces list ==========  ",len(faces_list))
