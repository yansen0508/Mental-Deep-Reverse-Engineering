...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep  5 16:21:04 2019
...
class testbase(BaseDataset):
    """docstring for CelebADataset"""
    def __init__(self):
        super(testbase, self).__init__()
        
    def initialize(self, opt):
        super(testbase, self).initialize(opt)
    ...
    def __getitem__(self, index):
        ...
        # load source image
        src_img = self.get_img_by_path(self.imgs_path[0])
        src_img_tensor = self.img2tensor(src_img)
        src_aus = self.get_aus_by_path(self.imgs_path[0])

        # load target image
        tar_img = self.get_img_by_path(img_path)
        tar_img_tensor = self.img2tensor(tar_img)
        tar_aus = self.get_aus_by_path(img_path)
        ...
        data_dict = {'src_img':src_img_tensor, 'src_aus':src_aus, 'tar_img':tar_img_tensor, 'tar_aus':tar_aus, \
                        'src_path':self.imgs_path[0], 'tar_path':img_path}
